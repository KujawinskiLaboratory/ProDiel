...
# Save plots or skip
Set to 1 if you want the script to save the plots, set to 0 if you do not want to save any plots.
```{r save plots setting}

should_save <- 1  # or 1 to save
...
# Save the plot
if (should_save) ggsave(filename, plot = cellCounts_time_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = cellCounts_extrap_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = cellCycle_time_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = light_time_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = FALS_time_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = Chl_time_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = cellCount_FALS_plot, width = 11, height = 8.5, dpi = 300)
...
# Save the plot
if (should_save) ggsave(filename, plot = combinedCellData_plot, width = 11, height = 8.5, dpi = 300)
...
intra_mq_DF10x_quant <- read.csv("riMAVEN15/Pro11_DF10x/Pro11_RP_filters_Altis_DF10x_MQ_mtabTable.2025.07.17.csv",check.names = FALSE)

intra_mq_DF10x_curve <- read.csv("riMAVEN15/Pro11_DF10x/Pro11_RP_filters_Altis_DF10x_MQ_curveInfo.2025.07.17.csv",check.names = FALSE)
...
intra_mq_DF500x_quant <- read.csv("riMAVEN15/Pro11_DF500x/Pro11_RP_filters_Altis_DF500x_MQ_mtabTable.2025.07.17.csv",check.names = FALSE)

intra_mq_DF500x_curve <- read.csv("riMAVEN15/Pro11_DF500x/Pro11_RP_filters_Altis_DF500x_MQ_curveInfo.2025.07.17.csv",check.names = FALSE)
...
# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_rawNoQC_nonZeroMtabs_nonNorm_plots_", today, "_bmg.pdf")

# Save the plot
if (should_save) pdf(filename, width = 11, height = 8.5)
...
# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_rawNoQC_nonZeroMtabs_nonNorm_overlaid_plots_", today, "_bmg.pdf")
...
Intra_zrb <- Intra_zr %>%
  mutate(df_adjusted_ngmL = case_when(dilution_factor == "df10x" ~ Concentration_ngmL* 10,
                                      dilution_factor =="df500x" ~ Concentration_ngmL * 500,
                                      TRUE ~ Concentration_ngmL), .after = Concentration_ngmL)%>%
  group_by(combined_mtab_ID,timepoint) %>%
  
  # Calculate mean_blank for blank rows only
  mutate(mean_blank = mean(df_adjusted_ngmL[type == "blank"], na.rm = TRUE)) %>%
  
  # Subtract mean_blank from each individual sample's concentration
  mutate(diff_sample_blank = ifelse(type == "sample", df_adjusted_ngmL - mean_blank, NA)) %>%
...
### Plot df corrected ng/mL with LOD
```{r metabolite MQ raw stacked plots}

# Create a PDF to save all plots

# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_dfAdjusted_ngmL_plots_wLOD_", today, "_bmg.pdf")

# Save the plot
if (should_save) pdf(filename, width = 11, height = 8.5)

# Get unique values of mtabName
unique_names <- Intra_zrb%>%
  arrange(mtabName)%>%
  select(mtabName)%>%
  unique()%>%
  deframe()
...
for (i in seq_along(unique_names)) {
  ...
  p <- Intra_zrb %>%
    mutate(timepoint = as.numeric(timepoint))%>%
    filter(mtabName == dynamic_title, type =="sample") %>%
    ggplot(aes(x = timepoint, y = df_adjusted_ngmL)) +
    geom_point(size=3) +
    geom_hline(aes(yintercept = LOD),color="red", linetype = "dashed")+
    geom_smooth(aes(x=timepoint,y=df_adjusted_ngmL),
                linewidth = 1, 
                color = "black",
                se = FALSE,
                method = "gam", formula = y ~ s(x, bs = "cs")) +
    stat_smooth(aes(x=timepoint,y=df_adjusted_ngmL),
                method = "gam", formula = y ~ s(x, bs = "cs"), 
                fill = "black",
                alpha = .3,
                linewidth = 1,
                geom = "ribbon")+
    labs(x = "timepoint", y = "DF Adjusted Concentration (ng/mL)", title = dynamic_title) +
...
Assuming 1 g/mL for density so filtered weight in grams equals the volume filtered for each sample. 
Multiply by two to account for the fact that we dilute our LC-MS sample by 2 (15 uL of sample + 15 uL of MQ water) and then multiply by 0.05 to account for the 50 uL of total reconstituion solvent added to the initial extract.
...
Blank subtracted concentration in ng/mL
...
Intra_zrbmndc <- Intra_zrbmndc%>%
  ungroup()%>%
  group_by(mtabName)%>%
  mutate(Blank_adj_concentration_inSample_nM =
           (Blank_adj_concentration_ngmL_inSample/MW)*1000,.after = Blank_adj_concentration_ngmL_inSample)%>%
  mutate(label = paste0(timepoint,replicateID))

```

## Cell count normalization
Cell counts are in cell/mL and concentration is in nM (nanomole/L) so need to
make sure units are the same and divide by 1000.
```{r normalziation}
...
Intra_fullQC <- Intra_fullQC%>%
  mutate(cellNorm_blank_adj_concen_inSample_agCell = ((Blank_adj_concentration_inSample_nM/cell_count/1000)*(10^18/10^9)))
...
group1_mtabs <- unique_mtabs[1:24]
group2_mtabs <- unique_mtabs[25:48]
...
p1 <- ggplot(summary_df, aes(x = timepoint.x, y = mean_val)) +
  ...
  scale_x_continuous(breaks = seq(from = 0, to = 46, by = 2)) +
  theme_classic(base_size = 14) +
  facet_wrap(~mtabName, scales = "free", ncol = 4, nrow = 6)
...
# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_QCd_cellNorm_mtab_barplots_first20_", today, "_bmg.png")

if (should_save) ggsave(filename,plot=p1,height=11,width=20,dpi=300)

# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_QCd_cellNorm_mtab_barplots_21to37_", today, "_bmg.png")

if (should_save) ggsave(filename,plot=p2,height=11,width=20,dpi=300)
...
# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_QCd_cellNorm_mtab_smoothedplots_first20_pointSmooth_", today, "_bmg.png")

if (should_save) ggsave(filename,plot=p1,height=11,width=20,dpi=300)

# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/figures/PD22_Pro11_QCd_cellNorm_mtab_smoothedplots_21to37_pointSmooth_", today, "_bmg.png")
...
# Save the plot
if (should_save) pdf(filename, width = 8.5, height = 11)
...
if (should_save) ggsave(filename,plot=fullQC_heatmap,height=11,width=8.5,units = "in",dpi=300)
...
x <- Intra_fullQC%>%
  select(Sample.Name,
         ...
         Blank_adj_concentration_inSample_nM,
         cellNorm_blank_adj_concen_inSample_agCell)

df_w_mtab_trends <- left_join(x,rain_24,by = "mtabName" )
...
# Generate the filename with the date automatically included
filename <- paste0("Scripts/R/Intracellular/r_csv_outputs/Pro11_Intra_withQC_cellNorm_mtabTable_wTemporalAnalysis_", today, "_bmg.csv")

if (should_save) write.csv(df_w_mtab_trends,filename)
...
### Heatmap of all significant metabolites
...
### Heatmap of significant metabolites found above LOD in all 24 timepoints (ntimepoint = 24)
```{r heatmap of significant diel metabolites}
...
scale_factor <- max(Intra_fullQC$cell_count_mean, na.rm = TRUE) / 200 # 200 = max RSD y-axis
...
# After you have your 48 metabolite names in order:
...
palette_48 <- c(
  ...
  "#ADFF2F", "#8A2BE2", "#DC143C", "#00CED1", "#FFD700", "#1E90FF", "#B22222", "#4B0082",
  "#DAA520", "#00FF7F", "#7FFF00", "#FF1493", "#A0522D", "#20B2AA", "#5F9EA0", "#8B008B",
  "#CD5C5C", "#FF4500", "#2E8B57", "#6A5ACD", "#FF6347", "#40E0D0", "#FF8C00", "#BA55D3",
  "#B0C4DE", "#3CB371", "#FA8072", "#4682B4", "#9ACD32", "#800000", "#4169E1", "#C71585",
  "#F08080", "#8FBC8F", "#9932CC", "#FF00FF", "#708090", "#87CEEB", "#6B8E23", "#483D8B"
)

# Attach names
names(palette_48) <- metabolite_names
...
stackedBar_cellCycle <- dom_cellCycle %>%
  ...
  scale_fill_manual(values = palette_48) +
...
if (should_save) pdf(filename, width = 11, height = 11)
...
x <- Intra_fullQC%>%
  select(label,cleanName,MW,r2_line,nPoints,LOD,LOQ,quantType,dilution_factor,n_timepoints,cellNorm_blank_adj_concen_inSample_agCell,timepoint.x)%>%
  mutate(timepoint.x = as.numeric(timepoint.x))%>%
...
if (should_save) write.csv(x,"Scripts/R/Intracellular/r_csv_outputs/PD22_Pro11_Intra_fullQC_mtabQuant_agCell_wCurveInfo_20250721.csv")

rm(x)

#Blank adjusted, volume normalized - nM
x<- Intra_fullQC%>%
  select(label,cleanName,MW,r2_line,nPoints,LOD,LOQ,quantType,dilution_factor,n_timepoints,Blank_adj_concentration_inSample_nM,timepoint.x)%>%
  mutate(timepoint.x = as.numeric(timepoint.x))%>%
  arrange(cleanName,timepoint.x)%>%
  select(-timepoint.x)%>%
  pivot_wider(names_from = label, values_from=Blank_adj_concentration_inSample_nM)

if (should_save) write.csv(x,"Scripts/R/Intracellular/r_csv_outputs/PD22_Pro11_Intra_fullQC_mtabQuant_nM_wCurveInfo_20250721.csv")

rm(x)

#Average cell normalized concentration (agCell) per timepoint + sdev

x <- Intra_fullQC %>%
  select(mtabName, timepoint.x, cellNorm_blank_adj_concen_inSample_agCell) %>%
  group_by(mtabName, timepoint.x) %>%
  summarise(
    mean = mean(cellNorm_blank_adj_concen_inSample_agCell, na.rm = TRUE),
    sd = sd(cellNorm_blank_adj_concen_inSample_agCell, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  # Ensure numeric ordering
  mutate(time_num = as.numeric(timepoint.x)) %>%
  pivot_longer(cols = c(mean, sd), names_to = "stat", values_to = "value") %>%
  mutate(timepoint_label = paste0("T", time_num, "_", stat)) %>%
  select(mtabName, time_num, timepoint_label, value) %>%
  arrange(mtabName, time_num) %>%
  select(-time_num) %>%
  pivot_wider(names_from = timepoint_label, values_from = value)

if (should_save) write.csv(x, file = "Scripts/R/Intracellular/r_csv_outputs/PD22_Pro11_Intra_fullQC_quantTable_meanSD_agCell_byTime_20250721.csv", row.names = FALSE)
...
nmds_data <- Intra_fullQC%>%
  select(label,timepoint.x,mtabName,cellNorm_blank_adj_concen_inSample_agCell)%>%
  mutate(timepoint.x = as.numeric(timepoint.x))%>%
  arrange(mtabName,timepoint.x)%>%
  rename(timepoint = timepoint.x)%>%
  pivot_wider(names_from = mtabName, values_from = cellNorm_blank_adj_concen_inSample_agCell)

nmds.mat <- as.matrix(nmds_data[,3:ncol(nmds_data)])
row.names(nmds.mat) <- nmds_data[[1]]

nmds.log <- log10(nmds.mat + 0.001)
nmds.log.shifted <- nmds.log - min(nmds.log)


set.seed(1)
dist <- vegdist(nmds.log.shifted,dist='bray')
...
nmds_df <- data.frame(nmds_data$timepoint,nmds$points)

nmds_plot <- ggscatter(nmds_df, x = "MDS1", y = "MDS2", color ="nmds_data.timepoint",size=3)+
